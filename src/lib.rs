...
///
/// This struct is created by the [median()](trait.MedianFilterable.html#tymethod.median) method on Iterator. See its documentation for more.
pub struct Median<I>
    where I: Iterator,
          I::Item: Clone + PartialOrd
{
    iterator: I,
    filter: Filter<<I as Iterator>::Item>,
}
...
{
    #[inline]
    fn new(iterator: I, filter_size: usize) -> Self {
        Median {
            iterator: iterator,
            filter: Filter::new(filter_size),
        }
    }
}

impl<I> Iterator for Median<I>
    where I: Iterator,
          I::Item: Clone + PartialOrd
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.iterator.next().map(|value| self.filter.consume(value))
    }
}

/// Trait to indicate what types are usable with the `Median` struct
pub trait MedianFilterable: Iterator + Sized
    where <Self as Iterator>::Item: Clone + PartialOrd
{
    /// Creates an iterator which applies a median filter of given window size on its values.
    ///
    /// The following:
    ///
    /// ```rust
    /// # use median::Filter;
    /// use median::MedianFilterable;
    ///
    /// let iter = vec![1, 2, 3, 4, 5].into_iter();
    /// let median = iter.median(3);
    /// ```
    ///
    ...
    /// let median = iter.map(|v| filter.consume(v));
    /// ```
    fn median(self, filter_size: usize) -> Median<Self>;
}

impl<I> MedianFilterable for I
    where I: Iterator,
          <I as Iterator>::Item: Clone + PartialOrd
{
    #[inline]
    fn median(self, filter_size: usize) -> Median<Self> {
        Median::new(self, filter_size)
    }
}
...
impl<T> ::std::fmt::Debug for ListNode<T>
where
    T: ::std::fmt::Debug,
{
    ...
    T: Clone + PartialOrd,
{
    ...
    #[inline]
    fn insert(&mut self, value: &T, current: usize) {
    ...
    #[inline]
    fn shift_median(&mut self, index: usize, current: usize) {
    ...
    #[inline]
    fn update_head(&mut self, value: &T) {
    ...
    #[inline]
    fn adjust_median_for_even_length(&mut self) {
        if self.len() % 2 == 0 {
            self.median = self.buffer[self.median].previous;
        }
    }
    ...
    #[inline]
    fn read_median(&self) -> T {
        self.buffer[self.median].value.clone().unwrap()
    }
}
